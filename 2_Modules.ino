...
/* Initializing variables for sensors*/
  ...
  float uviaCalc, uvibCalc, uvia, UV1Atot, uvib, UV1Btot, uvi, temperatures(64), avgTemp, hum, temp, IR1tot, Vis1tot, TotIrradiance1, LightExposure1, UV2Atot, UV2Btot,IR2tot, Vis2tot, TotIrradiance2, LightExposure2, e_stop, A; 
...
void buffValues2(void){ 
  int32_t lum = TSL2.getFullLuminosity();           //Set luminosity from TSL 1
  ir = lum >> 16;                                   //Set infrared from luminosity
  full = lum & 0xFFFF;                              //Set full from luminosity
  rawA = VEML2.rawUva();                            //Set rawA from VEML 1
  rawB = VEML2.rawUvb();                            //Set rawB from VEML 1
  visibleComp = VEML2.visibleCompensation();        //Set visible component from VEML 1
  irComp = VEML2.irCompensation();                  //Set infrared component from VEML 1
  MLX2.setCallibration(0);                          //Set calibration to 0 for MLX 1
  MLX2.measure(true);                               //Set measuro = true for MLX 1
  CalcVEML();                                       //Calculate function for VEML
...
void loop(void){                        //Main loop, runs continious until time cap is reached
  ...
  tcaselect(1);                         //Select port 1 on multiplexer
