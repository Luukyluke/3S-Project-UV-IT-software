...
void setup(){  
  ...
  //TRIAD3.enableBulb(AS7265x_LED_UV);
    ...
    tcaselect(3);
    if(!TSL4.begin()){
     Serial.println("Unable to communicate with TSL2591  4.... Check your wiring!");
     while(1);
    }
    if(!VEML4.begin()){
     Serial.println("Unable to communicate with VEML6075  4.... Check your wiring!");
     while(1);
    }  
    if(!TRIAD4.begin()){
     Serial.println("Unable to communicate with AS7265x  4.... Check your wiring!");
     while(1);
    }
  
  MLX4.setRefreshRate(RATE_512HZ);
  MLX4.setResolution(RES_18bit);
  MLX4.setEmissivity(0.95);
  MLX4.initialize(); 
 
  TRIAD4.setMeasurementMode(AS7265X_MEASUREMENT_MODE_6CHAN_CONTINUOUS);
  TRIAD4.setIntegrationCycles(10); 
  TRIAD4.disableIndicator();

  //Rather than toggle the LEDs with each measurement, turn on LEDs all the time
  //TRIAD4.enableBulb(AS7265x_LED_WHITE);
  //TRIAD4.enableBulb(AS7265x_LED_IR);
  //TRIAD4.enableBulb(AS7265x_LED_UV);
  
  delay(1000);
  Serial.println("ms,Hum1,Temp1,BP1,Min1,Max1,IR1,Visible1,Full1,Lux1,UV1A,UV1B,T1x1,T1x2,T1x3,T1x4,T1x5,T1x6,T1x7,T1x8,T1x9,T1x10,T1x11,T1x12,T1x13,T1x14,T1x15,T1x16,T1x17,T1x18,BP2,Min2,Max2,IR2,Visible2,Full2,Lux2,UV2A,UV2B,T2x1,T2x2,T2x3,T2x4,T2x5,T2x6,T2x7,T2x8,T2x9,T2x10,T2x11,T2x12,T2x13,T2x14,T2x15,T2x16,T2x17,T2x18,BP3,in3,Max3,IR3,Visible3,Full3,Lux3,UV3A,UV3B,3xA,T3xB,T3xC,T3xD,T3xE,T3xF,T3xG,T3xH,T3xI,T3xJ,T3xK,T3xL,T3xR,T3xS,T3xT,T3xU,T3xV,T3xW,ms,Hum1,Temp1,BP1,Min1,Max1,IR1,Visible1,Full1,Lux1,UV1A,UV1B,T1xA,T1xB,T1xC,T1xD,T1xE,T1xF,T1xG,T1xH,T1xI,T1xJ,T1xK,T1xL,T1xR,T1xS,T1xT,T1xU,T1xV,T1xW,BP2,Min2,Max2,IR2,Visible2,Full2,Lux2,UV2A,UV2B,T2xA,T2xB,T2xC,T2xD,T2xE,T2xF,T2xG,T2xH,T2xI,T2xJ,T2xK,T2xL,T2xR,T2xS,T2xT,Tx2U,T2xV,T2xW,BP3,in3,Max3,IR3,Visible3,Full3,Lux3,UV3A,UV3B,3xA,T3xB,T3xC,T3xD,T3xE,T3xF,T3xG,T3xH,T3xI,T3xJ,T3xK,T3xL,T3xR,T3xS,T3xT,T3xU,T3xV,T3xW,BP4,Min4,Max4,IR4,Visible4,Full4,Lux4,UV4A,UV4B,T4xA,T4xB,T4xC,T4xD,T4xE,T4xF,T4xG,T4xH,T4xI,T4xJ,T4xK,T4xL,T4xR,T4xS,T4xT,T4xU,T4xV,T4xW,");
 }
...
void buffValues4(){ 
  int32_t lum = TSL4.getFullLuminosity();
  ir = lum >> 16;
  full = lum & 0xFFFF;
  rawA = VEML4.rawUva();
  rawB = VEML4.rawUvb();
  visibleComp = VEML4.visibleCompensation();
  irComp = VEML4.irCompensation();
  MLX4.setCallibration(0);
  MLX4.measure(true);
  ...
  buffer += hum;                                  buffer += ",";
  buffer += temp;                                 buffer += ",";
  buffer += BP4.getThmc();                        buffer += ","; 
  buffer += MLX4.minTemp;                         buffer += ",";
  buffer += MLX4.maxTemp;                         buffer += ",";
  buffer += ir;                                   buffer += ",";
  buffer += (full - ir);                          buffer += ",";
  buffer += full;                                 buffer += ",";
  buffer += TSL4.calculateLux(full, ir), 6;       buffer += ",";
  buffer += uviaCalc;                             buffer += ",";
  buffer += uvibCalc;                             buffer += ",";
  buffer += TRIAD4.getCalibratedA();              buffer += ",";
  buffer += TRIAD4.getCalibratedB();              buffer += ",";
  buffer += TRIAD4.getCalibratedC();              buffer += ",";
  buffer += TRIAD4.getCalibratedD();              buffer += ",";
  buffer += TRIAD4.getCalibratedE();              buffer += ",";
  buffer += TRIAD4.getCalibratedF();              buffer += ",";
  buffer += TRIAD4.getCalibratedG();              buffer += ",";
  buffer += TRIAD4.getCalibratedH();              buffer += ",";
  buffer += TRIAD4.getCalibratedI();              buffer += ",";
  buffer += TRIAD4.getCalibratedJ();              buffer += ",";
  buffer += TRIAD4.getCalibratedK();              buffer += ",";
  buffer += TRIAD4.getCalibratedL();              buffer += ",";
  buffer += TRIAD4.getCalibratedR();              buffer += ",";
  buffer += TRIAD4.getCalibratedS();              buffer += ",";
  buffer += TRIAD4.getCalibratedT();              buffer += ",";
  buffer += TRIAD4.getCalibratedU();              buffer += ",";
  buffer += TRIAD4.getCalibratedV();              buffer += ",";
  buffer += TRIAD4.getCalibratedW();              buffer += ",";
}
void loop(void){  
